...
def _checkpoint_config(store: InMemoryStateStore) -> CheckpointConfig:
    capability_names = (
        "approval_provider",
...
def _server(
    *,
    ...
    agent: Agent,
    model_provider: Callable[[Agent, RunConfig], tuple[ScriptedLLM, ResolvedModelConfig]],
...
def test_turn_resume_rejects_new_input_and_foreign_turn() -> None:
    ...
    def model_provider(_agent: Agent, _config: RunConfig) -> tuple[ScriptedLLM, ResolvedModelConfig]:
...
def test_live_claim_returns_existing_owner_without_notifications_or_execution() -> None:
    ...
    thread_id, turn_id, _messages = _start_thread_and_turn(server, transport)
    checkpoint = store.load_checkpoint_v2(CHECKPOINT_KEY)
...
def test_reconciliation_resume_emits_canonical_interrupted_sequence() -> None:
    ...
    thread_id, turn_id, _messages = _start_thread_and_turn(server, transport)
    with store._lock:
        store._store_v2[CHECKPOINT_KEY].lease_expires_at_ms = 1
...
def _crashing_tool_server() -> tuple[AppServer, ChannelTransport, InMemoryStateStore, list[str]]:
    store = InMemoryStateStore()
    effects: list[str] = []
