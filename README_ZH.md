...
```bash
uv run vv-agent --prompt "概述一下这个框架" --backend moonshot --model kimi-k2.6

# 带每轮日志
uv run vv-agent --prompt "概述一下这个框架" --backend moonshot --model kimi-k2.6 --verbose
```
...
协议细节见 [docs/app-server.md](docs/app-server.md)，迁移清单见
[v-claw](docs/app-server-vclaw-migration.md) 与
[backend service](docs/app-server-backend-migration.md)。
...
schema。原有 `idempotency=` 参数仍是兼容别名；如果它与 typed metadata 都给出不同的
非 `unknown` 值，会在模型或工具操作前失败。未声明 typed metadata 且四个新拒绝项
保持默认值时，模型 schema 与 runtime 行为都不变。
...
- 编译映射：
  - `AgentCompiler` 将稳定的 Agent / Run 元数据转发到 `RuntimeTask`。
...
- 压缩事件：
  ...
  - 新的 `memory_compact_completed` producer 会携带实际执行的最强 mode
    ...
    `changed`。
  - 旧事件可以缺少这些新增字段；字段一旦出现，就必须满足严格类型和闭合 enum。
- Session Memory 行为：
...
| `V_AGENT_LIVE_MODEL` | `kimi-k2.6` | 模型名称 |
