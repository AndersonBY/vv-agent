...
CHECKPOINT_V2_SCHEMA = "vv-agent.checkpoint.v2"
...
class OperationJournalEntry:
    ...
    def from_dict(cls, payload: dict[str, Any]) -> OperationJournalEntry:
        ...
        return cls(
            ...
            idempotency_support=(
                payload.get("idempotency_support")
...
class CheckpointV2:
    ...
    schema_version: str = CHECKPOINT_V2_SCHEMA
    run_definition_schema: str = RUN_DEFINITION_V1_SCHEMA


@runtime_checkable
class CheckpointStoreV2(Protocol):
...
def validate_checkpoint_v2(checkpoint: CheckpointV2) -> None:
    if not isinstance(checkpoint, CheckpointV2):
        raise TypeError("checkpoint must be a CheckpointV2")
    ...
    if checkpoint.run_definition_schema != RUN_DEFINITION_V1_SCHEMA:
        raise CheckpointError(
    ...
    if not isinstance(checkpoint.messages, list) or not all(
        isinstance(item, Message) for item in checkpoint.messages
    ):
        raise TypeError("checkpoint messages must contain Message values")
    if not isinstance(checkpoint.cycles, list) or not all(
        isinstance(item, CycleRecord) for item in checkpoint.cycles
    ):
    ...
    canonical_json_bytes(checkpoint.unknown_fields, "checkpoint unknown_fields")
...
def claim_matches_v2(
    current: CheckpointV2 | None,
    ...
    return bool(
        ...
        and checkpoint_definition_matches_v2(current, snapshot)
    )


def checkpoint_definition_matches_v2(current: CheckpointV2, snapshot: CheckpointV2) -> bool:
    return bool(
...
def prepare_claimed_terminal_v2(
    ...
    *,
    ...
    validate_checkpoint_v2(terminal)
    return terminal


def prepare_event_delivery_v2(
    current: CheckpointV2,
    *,
    event_id: str,
    payload_digest: str,
    ...
    validate_checkpoint_v2(delivered)
    return delivered
