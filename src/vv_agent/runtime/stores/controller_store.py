"""Controller snapshot transitions and locked memory-store persistence."""
...
def prepare_notification_claim(
    ...
    *,
    ...
    if row["outbox_state"] == "claimed" and row["claim_token"] != claim_token and (row["lease_expires_at_ms"] or 0) > now_ms:
    ...
    staged["attempt"] += 1
    _checked_notification(staged)
    return staged


def prepare_notification_completion(
    row: dict[str, Any] | None,
    *,
    payload_digest: str,
    claim_token: str,
...
def prepare_notification_reconciliation(
    row: dict[str, Any] | None,
    *,
    payload_digest: str,
    outcome: str,
...
def prepare_host_response_reap(
    ...
    *,
    ...
    if record is None or checkpoint is None or record["checkpoint_key"] != checkpoint_key:
        return None
    _checked_host_record(record, checkpoint_key=checkpoint_key)
    ...
    _checked_host_record(staged, checkpoint_key=checkpoint_key)
    return staged


def _validate_command_binding(current: Checkpoint | None, command: ControllerCommand, *, lease_now_ms: int) -> Checkpoint:
    if current is None:
...
def _append_resume_event(checkpoint: Any, *, event_id: str, created_at: float) -> None:
...
def prepare_controller_command(
    ...
    *,
    ...
    event_id: str,
    created_at: float,
) -> tuple[Checkpoint, dict[str, Any] | None, ControllerCommandReceipt]:
    checkpoint = clone_checkpoint(_validate_command_binding(current, command_value, lease_now_ms=now_ms))
    ...
    if kind == "host_interaction_response":
        ...
        if not isinstance(active, dict):
            raise _controller_error("host interaction response has no pending interaction", "host_interaction_recovery_required")
        ...
        checkpoint.revision += 1
        _append_control_events(checkpoint, state=AgentStatus.SUSPENDED.value, event_id=event_id, created_at=created_at)
        resulting_status = AgentStatus.SUSPENDED.value
    elif kind == "resume":
        ...
        if origin["status"] == AgentStatus.DEFERRED.value and any(
            ...
            wake_destination = "distributed_advance"
        checkpoint.revision += 1
        _append_resume_event(checkpoint, event_id=event_id, created_at=created_at)
        resulting_status = checkpoint.status.value
    elif kind == "cancel":
        if checkpoint.claim_token is not None:
            ...
            if not claim_expired:
                ...
                terminal_candidate.terminal_result = _terminal_result(
                    authority,
            ...
            checkpoint.terminal_result = _terminal_result(
                checkpoint,
                reason=CompletionReason.CANCELLED,
                error={
                    "code": "cancelled_with_unknown_outcome",
            ...
            checkpoint.claimed_cycle = None
            checkpoint.lease_expires_at_ms = None
            checkpoint = prepare_unclaimed_terminal(checkpoint, created_at=created_at)
            checkpoint.revision += 1
            _append_control_events(
                checkpoint,
                state=AgentStatus.FAILED.value,
    ...
    return checkpoint, staged_record, receipt
...
def _validate_producer_checkpoint(
    current: Checkpoint | None,
...
def _outcome_for_record(
    record: dict[str, Any], notification: dict[str, Any] | None, *, status: str, checkpoint_revision: int
) -> HostInteractionOutcome:
    _checked_host_record(record, checkpoint_key=str(record["checkpoint_key"]))
    ...
    _checked_notification(notification)
...
def prepare_host_interaction(
    ...
    *,
    admission_context: HostInteractionAdmissionContext,
    created_at: float,
) -> tuple[Checkpoint, dict[str, Any], dict[str, Any], HostInteractionOutcome]:
    current = _validate_producer_checkpoint(current, request_value, admission_context=admission_context)
    key, revision = admission_context.checkpoint_key, current.revision
    ...
    notification_payload = _notification_payload(record_id, request_value, notification_id)
    ...
    record = _record(checkpoint_key=key, request=request_value, record_id=record_id)
    notification = {
        "notification_id": notification_id,
        "checkpoint_key": key,
        "record_id": record_id,
...
def prepare_host_response_consumption(
    checkpoint: Checkpoint | None,
    record: dict[str, Any] | None,
    ...
    *,
    now_ms: int | None,
    created_at: float,
) -> tuple[Checkpoint, dict[str, Any], HostInteractionRecoveryResult]:
    record_id, checkpoint_key = envelope_value.record_id, envelope_value.checkpoint_key
    ...
    if record["state"] != "resolved_pending":
        raise _controller_error("host interaction response is not ready for recovery", "host_interaction_recovery_required")
    if checkpoint.revision != envelope_value.expected_revision or checkpoint.resume_attempt != envelope_value.resume_attempt:
    ...
    if (
        ...
        raise _controller_error("host interaction recovery checkpoint is not executable", "host_interaction_recovery_stale")
    if checkpoint.claim_token is not None or checkpoint.claimed_cycle is not None or checkpoint.lease_expires_at_ms is not None:
        raise _controller_error(
...
class ControllerStoreMixin:
    ...
    def get_host_interaction_notification(self, notification_id: str) -> dict[str, Any] | None:
        ...
        with self._lock:
            ...
            if row is None:
                return None
            return deepcopy(_checked_notification(row))
        ...
        with self._lock:
            for (key, _interaction_id), record in self._host_interaction_records.items():
                if key == checkpoint_key and record["state"] == "resolved_pending":
                    return deepcopy(_checked_host_record(record, checkpoint_key=checkpoint_key))
        return None
        ...
        *,
        ...
        with self._lock:
            ...
            if existing is not None:
                _checked_host_record(existing, checkpoint_key=key)
            ...
            snapshot, record, notification, outcome = prepare_host_interaction(
                self._load_controller_checkpoint(key),
                request_value,
    ...
    def _admit_controller_command(self, command: ControllerCommand | Mapping[str, Any]) -> tuple[ControllerCommandReceipt, bool]:
        ...
        with self._lock:
            ...
            if (
                current is not None
                and command_value.kind in {"resume", "host_interaction_response"}
                and isinstance(current.suspended_origin, dict)
            ):
                active = current.suspended_origin.get("active_host_interaction")
            record = (
                self._host_interaction_records.get((command_value.handle.checkpoint_key, active["interaction_id"]))
                if isinstance(active, dict)
                else None
            )
            checkpoint, staged_record, receipt = prepare_controller_command(
                current,
                record,
                command_value,
                now_ms=now_ms,
    ...
    def claim_and_consume_host_interaction_response(self, envelope: Mapping[str, Any]) -> HostInteractionRecoveryResult:
        ...
        with self._lock:
            ...
            snapshot, staged_record, return_value = prepare_host_response_consumption(
                checkpoint,
                record,
                envelope_value,
                now_ms=self._lease_now_ms(None),
            ...
            if return_value.kind == "replayed":
                return return_value
            record_key = (checkpoint_key, envelope_value.interaction_id)
            previous_checkpoint = self._store.get(checkpoint_key)
            previous_record = self._host_interaction_records.get(record_key)
            try:
    ...
    def reap_host_interaction_record(self, *, record_id: str, checkpoint_key: str, now_ms: int) -> bool:
        ...
        with self._lock:
            ...
            checkpoint = self._load_controller_checkpoint(checkpoint_key)
            staged = prepare_host_response_reap(checkpoint, record, checkpoint_key=checkpoint_key, now_ms=now_ms)
            if staged is None:
                return False
            self._host_interaction_records[(checkpoint_key, staged["interaction_id"])] = staged
            return True
    ...
    def claim_host_interaction_notification(
        ...
        *,
        ...
        with self._lock:
            row = self._host_interaction_notifications.get(notification_id)
            updated = prepare_notification_claim(
                row,
            ...
            if updated is not None and updated is not row:
                self._host_interaction_notifications[notification_id] = updated
    ...
    def complete_host_interaction_notification(
        ...
        *,
        ...
        with self._lock:
            row = self._host_interaction_notifications.get(notification_id)
            updated = prepare_notification_completion(
                row,
                payload_digest=payload_digest,
                claim_token=claim_token,
                attempt=attempt,
    ...
    def reconcile_host_interaction_notification(
        ...
        *,
        ...
        with self._lock:
            row = self._host_interaction_notifications.get(notification_id)
            updated = prepare_notification_reconciliation(
                row,
                payload_digest=payload_digest,
                outcome=outcome,
                now_ms=now_ms,
