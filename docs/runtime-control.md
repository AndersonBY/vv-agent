...
An admitted deferred tool uses `tool_call_deferred` in place of the completed
event. It carries only the opaque framework handle, has `duration_ms: null`,
and never projects a model-visible tool result. The eventual definitive
`SUCCESS` or `ERROR` resolution emits one stable `tool_call_completed` event.
...
checkpoint v12 operation journal and deferred barrier.
...
The event wire version is `v5`, used as a strict discriminator rather than a
