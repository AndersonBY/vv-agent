...
| Contract 21 Bash management receipts, original execution deadline, task/workspace ownership and confirmed stop | `src/vv_agent/runtime/background_sessions.py`, `src/vv_agent/runtime/processes.py`, `src/vv_agent/tools/handlers/bash.py`, `src/vv_agent/tools/handlers/background.py`; `tests/test_bash_process_management.py`, `tests/test_bash_tools.py` |
...
- Python exposes `DistributedRunHandle`, `DistributedDeliveryOutcome`, and
  ...
  `execute_local()` is a thin adapter for immediate or completed worker
  responses and does not poll a distributed run.
